...
def train(args):
    ...
    if im_dataset.num_classes > 0:
        dit_model_config['num_classes'] = im_dataset.num_classes
        print(f'Using {im_dataset.num_classes} classes from dataset')
        print(f'Class mapping: {im_dataset.class_to_id}')
    else:
        print(f'No labels loaded, using num_classes from config: {dit_model_config["num_classes"]}')
    ...
    training_state_path = os.path.join(train_config['task_name'], 'dit_training_state.pth')
    if os.path.exists(training_state_path):
        training_state = torch.load(training_state_path, map_location=device)
        start_epoch = training_state['epoch'] + 1
        optimizer.load_state_dict(training_state['optimizer'])
        print(f'Resuming DiT training from epoch {start_epoch}')
    ...
    for epoch_idx in range(start_epoch, num_epochs):
        ...
        for batch in tqdm(data_loader):
            step_count += 1
            # Unpack batch: (image/latent, class_label)
            im, class_labels = batch
            ...
            pred = model(noisy_im, t, class_labels)
        ...
        torch.save(training_state, os.path.join(train_config['task_name'], 'dit_training_state.pth'))
